...
// Who is allowed to edit this page.
//
...
// visitor to press the button owns your page. Open /admin and claim it now.
...
export function OwnerGate({
  ...
  const resolve = useCallback(async () => {
    ...
    const { data: rows } = await backend.database
      ...
      .select("user_id")
      .limit(1);
    const owner = (rows as { user_id: string | null }[] | null)?.[0];
  ...
  async function claim() {
    ...
    if (error) {
      ...
      setProblem(
        error.code === "23505"
          ? "This page was just claimed by someone else."
          : error.message,
      );
    }
    await resolve();
  ...
  if (status.state === "signed-out") {
    return (
      <Centered>
        <p className="text-base text-foreground">Sign in to edit this page.</p>
        <Primary onClick={() => void signIn()}>Sign in with Google</Primary>
    ...
    return (
      <Centered>
        <p className="text-base text-foreground">
          This page does not have an owner yet.
        </p>
        <p className="max-w-sm text-sm leading-6">
          Claim it as {status.user.email} and only you will be able to edit it.
          This cannot be undone or transferred from here.
        </p>
        <Primary disabled={claiming} onClick={() => void claim()}>
  ...
  if (status.state === "not-owner") {
    return (
      <Centered>
        ...
        <button
          className="h-11 rounded-full border border-line px-6 text-sm transition-colors hover:bg-card"
